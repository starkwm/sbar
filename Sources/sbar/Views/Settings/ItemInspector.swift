    ...
    Form {
      ...
      Section("Refresh") {
        Picker(
          "Policy",
          selection: Binding(
            get: { item.refresh?.mode },
            set: { mode in
              item.refresh = mode.map {
                RefreshPolicy(mode: $0, seconds: $0 == .interval ? 5 : nil)
              }
            }
          )
        ) {
        ...
        if item.refresh?.mode == .interval {
          TextField(
            "Seconds",
            value: Binding(get: { item.refresh?.seconds }, set: { item.refresh?.seconds = $0 }),
            format: .number
          )
        }
        if item.refresh != nil {
          TextField(
            "Trigger event",
            text: Binding(
              get: { item.refresh?.event ?? "" },
              set: { item.refresh?.event = $0.isEmpty ? nil : $0 }
            )
          )
        }
      }
      ...
      if item.type == .command {
        Section("Command") {
          TextField(
            "Shell script",
            text: Binding(
              get: { item.command?.script ?? "" },
              set: { item.command = command(script: $0) }
            ),
            axis: .vertical
          )
          TextField(
            "Interval (seconds; blank runs once)",
            value: Binding(
              get: { item.command?.interval },
              set: {
                var c = command()
                c.interval = $0
                item.command = c
          ...
          TextField(
            "Trigger event",
            text: Binding(
              get: { item.command?.event ?? "" },
              set: {
                var c = command()
                c.event = $0.isEmpty ? nil : $0
                item.command = c
              }
            )
          )
      ...
      DisclosureGroup("Advanced item JSON (including children)") {
        JSONEditor(value: $item).id(item.id)
      }
    }.formStyle(.grouped)
  }
...
private struct ActionEditor: View {
  ...
  var body: some View {
    Toggle(
      ...
      isOn: Binding(
        get: { action != nil },
        set: { action = $0 ? ItemAction(kind: .url, value: "") : nil }
      )
    )
    if action != nil {
      Picker(
        "Action",
        selection: Binding(get: { action?.kind ?? .url }, set: { action?.kind = $0 })
      ) {
