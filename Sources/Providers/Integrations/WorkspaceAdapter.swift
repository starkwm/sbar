...
struct WorkspaceAdapter {
    ...
    static func yabaiLabel(_ data: Data) throws -> String {
        struct Space: Decodable { let index: Int; let label: String? }
    ...
    static func query(_ type: ItemType) async throws -> String {
        ...
        let arguments = type == .aerospace ? ["list-workspaces", "--focused"] : ["-m", "query", "--spaces", "--space"]
        let result = try await CommandRunner.run(executable: path, arguments: arguments, timeout: 2)
