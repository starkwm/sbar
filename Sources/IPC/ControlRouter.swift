...
final class ControlRouter {
    ...
    func handle(_ request: ControlRequest) -> ControlResponse {
        do {
            ...
            case "query":
                return ControlResponse(value: try JSONDecoder().decode(JSONValue.self, from: JSONEncoder().encode(store.configuration)))
            case "reload":
                store.load()
            ...
            case "trigger":
                ...
                return ControlResponse()
            case "set":
                guard request.arguments.count == 2, let value = request.value else { throw SocketFailure.message("Expected item ID, property and value.") }
                let id = request.arguments[0]
                let property = request.arguments[1]
                guard ["label", "symbol", "enabled", "priority", "format", "style", "popup"].contains(property) else { throw SocketFailure.message("Property cannot be changed at runtime.") }
                ...
                func update(_ items: inout [ItemConfiguration]) throws -> Bool {
                    for index in items.indices {
                        if items[index].id == id {
                            ...
                            guard case var .object(object) = try JSONDecoder().decode(JSONValue.self, from: encoded) else { return false }
                            object[property] = value
                            items[index] = try JSONDecoder().decode(ItemConfiguration.self, from: JSONEncoder().encode(JSONValue.object(object)))
                            return true
                        }
                ...
                let found = try update(&candidate.items.left) || update(&candidate.items.center) || update(&candidate.items.right)
