...
final class ControlServer: @unchecked Sendable {
        ...
        try queue.sync {
            ...
            do {
                ...
                if lstat(path, &info) == 0 {
                    guard info.st_uid == getuid(), info.st_mode & S_IFMT == S_IFSOCK else { throw SocketFailure.message("Refusing to replace a non-socket or another user's socket.") }
                    unlink(path)
                }
    ...
    func publish(_ response: ControlResponse) {
        queue.async { [weak self] in
            ...
            for fd in self.connections.keys.filter({ self.connections[$0]?.subscribed == true }) { self.respond(response, to: fd, closeAfter: false) }
        }
    }
    ...
    private func acceptClients(_ fd: Int32) {
        while true {
            ...
            queue.asyncAfter(deadline: .now() + 5) { [weak self] in
                guard self?.connections[client]?.id == id, self?.connections[client]?.subscribed == false else { return }
    ...
    private func readClient(_ fd: Int32) {
        ...
        guard count > 0 else { if count == 0 || errno != EAGAIN { disconnect(fd) }; return }
        guard connections[fd]?.handling == false else { disconnect(fd); return }
        connections[fd]?.data.append(contentsOf: buffer.prefix(count))
        guard let connection = connections[fd] else { return }
        guard connection.data.count <= 131_072 else { disconnect(fd); return }
        guard let newline = connection.data.firstIndex(of: 10) else { return }
        let id = connection.id
        connections[fd]?.handling = true
        do {
            let request = try JSONDecoder().decode(ControlRequest.self, from: connection.data.prefix(upTo: newline))
