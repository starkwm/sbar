...
struct PluginProcess {
    static func run(configuration: PluginConfiguration, mailbox: PluginMailbox, output: @escaping @Sendable (String) -> Void) async throws {
        let task = Task.detached { try execute(configuration: configuration, mailbox: mailbox, output: output) }
        try await withTaskCancellationHandler { try await task.value } onCancel: { task.cancel() }
    }

    private static func execute(configuration: PluginConfiguration, mailbox: PluginMailbox, output: @escaping @Sendable (String) -> Void) throws {
        var input: [Int32] = [0, 0]
        ...
        while !Task.isCancelled {
            ...
            if !pendingInput.isEmpty {
                ...
                else if count < 0 && errno != EAGAIN && errno != EINTR { pendingInput.removeAll() }
            }
            ...
            if count > 0 {
                ...
                while let newline = pendingOutput.firstIndex(of: 10) {
                    let message = try JSONDecoder().decode(PluginOutput.self, from: pendingOutput.prefix(upTo: newline))
            ...
            if exited && count <= 0 {
                ...
                if status != 0 { throw CommandFailure.exitStatus(status & 0x7f == 0 ? (status >> 8) & 0xff : 128 + (status & 0x7f)) }
                return
            }
