...
func main() throws {
  ...
  if arguments.first == "--socket" {
    guard arguments.count >= 3 else {
      throw SocketFailure.message("--socket requires a path and command")
    }
    path = arguments[1]
  ...
  case "trigger":
    guard (1...2).contains(arguments.count) else {
      throw SocketFailure.message("trigger requires an event and optional JSON")
    }
    request.arguments = [arguments[0]]
    if arguments.count == 2 {
      request.value = try JSONDecoder().decode(JSONValue.self, from: Data(arguments[1].utf8))
    }
  case "set":
    guard arguments.count == 3 else {
      throw SocketFailure.message("set requires item, property and value")
    }
    request.arguments = Array(arguments.prefix(2))
    request.value =
      (try? JSONDecoder().decode(JSONValue.self, from: Data(arguments[2].utf8)))
      ?? .string(arguments[2])
