...
struct InteractiveItemView: View {
    ...
    var body: some View {
        Group {
            if item.type == .group {
                ...
                } label: { BarItemView(configuration: item) }
                .buttonStyle(.plain)
            } else {
        ...
        .contextMenu {
            if let action = item.secondaryAction { Button("Run secondary action") { actions.run(action) } }
        }
    ...
    private var interactive: Bool { item.primaryAction != nil || item.secondaryAction != nil || item.popup != nil || item.type == .popup }
}
