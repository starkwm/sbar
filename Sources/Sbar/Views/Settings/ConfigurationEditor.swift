...
final class ConfigurationEditor {
  ...
  func items(in section: Section) -> [ItemConfiguration] {
    switch section {
    case .left: draft.items.left
    case .center: draft.items.center
    case .right: draft.items.right
    }
  }

  func replace(_ items: [ItemConfiguration], in section: Section) {
    switch section {
    case .left: draft.items.left = items
    case .center: draft.items.center = items
    case .right: draft.items.right = items
    }
  }
  ...
  func remove(_ id: String) {
    for section in Section.allCases {
      replace(items(in: section).filter { $0.id != id }, in: section)
    }
  ...
  func move(_ id: String, to section: Section, before target: String? = nil) {
    guard id != target,
      let item = (draft.items.left + draft.items.center + draft.items.right).first(where: {
        $0.id == id
      })
    else { return }
    remove(id)
    ...
    let index =
      target.flatMap { target in destination.firstIndex { $0.id == target } }
