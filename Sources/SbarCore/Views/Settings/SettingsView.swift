...
struct SettingsView: View {
  ...
  var body: some View {
    ...
    VStack(spacing: 0) {
      BarView(configuration: editor.validationError == nil ? editor.draft : editor.baseline)
        .frame(
          height: editor.draft.bar.height.isFinite ? min(96, max(20, editor.draft.bar.height)) : 32
        )
                  ...
                  Text(item.label ?? item.id).tag(item.id)
                    ...
                    .contextMenu {
                      ...
                      ForEach(ConfigurationEditor.Section.allCases) { destination in
                        Button("Move to \(destination.rawValue)") {
                          editor.move(item.id, to: destination)
                        }
                      }
                    }
                }
      ...
      if editor.dirty && store.configuration != editor.baseline {
        Text("The live configuration changed. Reload to discard this draft, or save to replace it.")
          .foregroundStyle(.orange).padding(8)
      }
      HStack {
        Button("Reload") {
          reload()
          editor.load(store.configuration)
        }
  ...
  private var selectedItem: Binding<ItemConfiguration>? {
    for section in ConfigurationEditor.Section.allCases {
      if let index = editor.items(in: section).firstIndex(where: { $0.id == editor.selection }) {
        ...
        return Binding(
          get: { editor.items(in: section).first(where: { $0.id == original.id }) ?? original },
          set: { item in
  ...
  private var themeForm: some View {
    Form {
      Picker("Position", selection: $editor.draft.bar.position) {
        Text("Top").tag(BarPosition.top)
        Text("Bottom").tag(BarPosition.bottom)
      }
      Picker("Displays", selection: $editor.draft.bar.displays) {
        Text("Main").tag(DisplaySelection.main)
        Text("All").tag(DisplaySelection.all)
        Text("Selected").tag(DisplaySelection.selected)
      }
      if editor.draft.bar.displays == .selected {
        ForEach(NSScreen.screens, id: \.self) { screen in
          ...
          {
            Toggle(
              "\(screen.localizedName) (\(number.uint32Value))",
              isOn: Binding(
                get: {
      ...
      Picker(
        "Window level",
        selection: Binding(
          get: { editor.draft.bar.windowLevel ?? .statusBar },
          set: { editor.draft.bar.windowLevel = $0 }
        )
      ) {
        ForEach(BarWindowLevel.allCases, id: \.self) { Text($0.rawValue).tag($0) }
      }
      Toggle(
        "Pass mouse through empty regions",
        isOn: Binding(
          get: { editor.draft.bar.mousePassThrough ?? false },
          set: { editor.draft.bar.mousePassThrough = $0 }
        )
      )
      ...
      Section("Bar theme") {
        TextField(
          "Background",
          text: Binding(
            get: { editor.draft.theme?.background ?? "" },
            set: {
              var theme = editor.draft.theme ?? BarTheme()
              theme.background = $0.isEmpty ? nil : $0
      ...
      Section("Default item style") {
        StyleEditor(
          style: Binding(
            get: { editor.draft.theme?.itemStyle ?? ItemStyle() },
            set: {
              var theme = editor.draft.theme ?? BarTheme()
              theme.itemStyle = $0
              editor.draft.theme = theme
  ...
  private func themeNumber(_ key: WritableKeyPath<BarTheme, Double?>) -> Binding<Double?> {
    Binding(
      get: { editor.draft.theme?[keyPath: key] },
      set: { value in
        var theme = editor.draft.theme ?? BarTheme()
        theme[keyPath: key] = value
        editor.draft.theme = theme
      }
    )
  }
