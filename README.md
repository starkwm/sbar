...
`plugin.symbols.running`, `.success`, and `.failure` supply state-specific symbols, with shared `symbols.font` and `.size` for custom glyphs. A top-level item `symbol` overrides the state symbol, which overrides the result symbol. `plugin.tints.running`, `.success`, and `.failure` override result tints; unspecified colors inherit the item style. `showValue:false` hides text and `showSymbol:false` hides symbols; both default to `true`. Display text collapses whitespace to a single line and is limited by `plugin.maxLength` (default 4096, range 1–4096), with an ellipsis when truncated.
