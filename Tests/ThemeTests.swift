...
struct ThemeTests {
    ...
    func inheritance() {
        let theme = ItemStyle(tint: "#112233", background: "#223344", fontSize: 18, fontWeight: .bold, horizontalPadding: 8, verticalPadding: 4, cornerRadius: 6)
    ...
    func defaults() throws {
        let config = try JSONDecoder().decode(BarConfiguration.self, from: Data(#"{"schemaVersion":1,"bar":{},"items":{}}"#.utf8))
    ...
    func roundTrip() throws {
        let json = ##"{"schemaVersion":1,"bar":{},"theme":{"itemSpacing":5,"itemStyle":{"tint":"#aabbcc","fontWeight":"semibold"}},"items":{"right":[{"id":"clock","type":"clock","style":{"fontSize":16,"background":"#11223380"}}]}}"##
        ...
        #expect(try JSONDecoder().decode(BarConfiguration.self, from: JSONEncoder().encode(config)) == config)
    }
    ...
    @Test("Malformed colors are rejected", arguments: ["red", "#fff", "#gg0000", "#1234567", "123456", "#１２３４５６"])
    func invalidColors(value: String) {
        ...
        #expect(throws: ConfigurationError.invalidStyle(path: "theme.background", reason: "Use #RRGGBB or #RRGGBBAA.")) {
            try config.validate()
        }
    }
    ...
    func invalidItem() {
        ...
        #expect(throws: ConfigurationError.invalidStyle(path: "items.right[1].style.fontSize", reason: "Must be between 8.0 and 72.0.")) {
            try config.validate()
        }
    }

    @Test("Nonfinite and negative spacing is rejected", arguments: [-1.0, Double.infinity, Double.nan])
