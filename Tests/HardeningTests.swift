...
struct HardeningTests {
    ...
    func arguments() throws {
        #expect(try StartupOptions.parse(["--config", "/tmp/custom.json"]).configurationURL.path == "/tmp/custom.json")
        #expect(throws: (any Error).self) { try StartupOptions.parse(["--config"]) }
        #expect(throws: (any Error).self) { try StartupOptions.parse(["--config="]) }
        #expect(throws: (any Error).self) { try StartupOptions.parse(["--config", "/a", "--config", "/b"]) }
    }

    @Test("Version one commands migrate recursively without rewriting files")
    func migration() throws {
        let json = #"{"schemaVersion":1,"bar":{},"items":{"left":[{"id":"group","type":"group","children":[{"id":"c","type":"command","command":{"script":"date","interval":12,"event":"refresh"}}]}]}}"#
    ...
    func cosmeticChanges() async throws {
        ...
        let providers = ProviderRegistry()
        defer { providers.stop(); try? FileManager.default.removeItem(at: url) }
        let command = CommandConfiguration(script: "printf x >> '\(url.path)'; printf ready")
        var config = BarConfiguration(bar: .init(), items: .init(left: [.init(id: "cmd", type: .command, command: command)]))
        providers.configure(config)
        let deadline = ContinuousClock.now.advanced(by: .seconds(2))
        while providers.itemValues["cmd"] != "ready" && ContinuousClock.now < deadline { try await Task.sleep(for: .milliseconds(20)) }
    ...
    func formatting() {
        ...
        #expect(ClockText.string(date, format: "HH:mm:ss", timeZone: TimeZone(secondsFromGMT: 0)!) == "13:05:09")
        #expect(ActionRunner.expand("${ROOT}/tool", environment: ["ROOT": "${OTHER}", "OTHER": "/tmp"]) == "${OTHER}/tool")
    }
