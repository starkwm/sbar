...
struct ControlTests {
    ...
    func socket() throws {
        ...
        let server = ControlServer(path: path) { request in ControlResponse(value: .string(request.command)) }
        try server.start()
        defer { server.stop() }
        ...
        #expect(count > 0)
        let response = try JSONDecoder().decode(ControlResponse.self, from: Data(buffer.prefix(max(0, count))))
        #expect(response.ok)
    ...
    func routing() {
        let store = ConfigurationStore(configurationURL: URL(fileURLWithPath: "/tmp/not-created-\(UUID().uuidString).json"))
        let events = EventBus()
        let router = ControlRouter(store: store, providers: ProviderRegistry(), events: events)
        #expect(router.handle(ControlRequest(command: "set", arguments: ["clock", "enabled"], value: .bool(false))).ok)
        #expect(store.configuration.items.right.last?.enabled == false)
        #expect(!router.handle(ControlRequest(command: "set", arguments: ["clock", "priority"], value: .string("bad"))).ok)
        #expect(!FileManager.default.fileExists(atPath: store.configurationURL.path))
        #expect(router.handle(ControlRequest(command: "trigger", arguments: ["refresh"], value: .number(3))).ok)
