...
public enum SocketFailure: Error, LocalizedError {
    case message(String)
    public var errorDescription: String? { if case let .message(message) = self { message } else { "Socket error" } }
}
...
public enum LocalSocket {
    ...
    public static func address<T>(_ path: String, body: (UnsafePointer<sockaddr>, socklen_t) throws -> T) throws -> T {
        ...
        guard bytes.count <= MemoryLayout.size(ofValue: address.sun_path) else { throw SocketFailure.message("Socket path is too long.") }
