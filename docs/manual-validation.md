...
2. Save a temporary configuration, then launch with `.build/debug/sbar --config /tmp/starkbar-check/config.json`. Use `.build/debug/sbar query --socket /tmp/starkbar-check/control.sock` for that instance. Quit any existing instance using the same config directory first.
...
9. Exercise `sbar set`, `trigger`, and `subscribe`; confirm transient edits persist only after Settings Save. Test a command timeout and a plugin that crashes or emits malformed JSON. Verify the app stays responsive and the process exits on removal/quit.
