...
1. Run `make test`, then `make`. Launch `.build/debug/StarkBar` and open Settings from its menu extra.
2. Save a temporary configuration, then launch with `.build/debug/StarkBar --config /tmp/starkbar-check/config.json`. Use `.build/debug/barctl --socket /tmp/starkbar-check/control.sock query` for that instance. Quit any existing instance using the same config directory first.
