...
by default. Choose its displayed values with top-level `text`:
...
Raw provider events retain used-byte formatting regardless of an item's text
template. Refresh policies capture the full numeric state, including unavailable
