...
`plugin.symbols.running`, `.success`, and `.failure` supply state-specific symbols, with shared `symbols.font` and `.size` for custom glyphs. A top-level item `symbol` overrides the state symbol, which overrides the result symbol. `plugin.tints.running`, `.success`, and `.failure` override result tints; unspecified colors inherit the item style. Top-level `text: ""` hides text. `showSymbol:false` hides symbols; it defaults to `true`. Display text collapses whitespace to a single line and is limited by `plugin.maxLength` (default 4096, range 1–4096), with an ellipsis when truncated. The limit applies before the item's [text template](../text-templates.md) renders; added template text is not included.

Use the item's top-level `text` to format `{{value}}`, `{{status}}`, and `{{error}}`. The plugin message's `text` supplies provider data and is never evaluated as a template.
